...
# Done
@api_view(['POST'])
def get_customers_by_username(request):
    username =request.data['username']
    serusername= UserSerializer().get_User_by_username(username)
    userid= serusername['user_id']
    obj= Customer.objects.filter(user_id = userid)
    return Response(CustomerSerializer().get_Customers_by_username(obj))
...
def Airline_Companies(request,id=-1):
    ...
    # if request.method == 'POST': #method post add new row
    #     idofAirlineCompfromtoken= 1 #add data from token
    #     idofContrfromtoken= 3 #add data from token
    #     Airline_Companie.objects.create(company_name =request.data['company_name'],
    #     country_id=(Countrie.objects.get(_id = idofContrfromtoken)),
    #     user_id=User.objects.get(id = idofAirlineCompfromtoken))
...
@api_view(['GET'])
# Done
def get_airline_by_user_id(request,id):
    if request.method == 'GET':
        # name = request.data['company_name']
        return Response(Airline_CompaniesSerializer().get_airline_company_by_user_id(user_id=id))
...
def get_flights_by_parameters(request):
    if request.method == 'POST': #method post add new row
        ...
        # print(origin_country_id,destination_country_id,departure_time)
        object= Flight.objects.filter(origin_country_id= origin_country_id,destination_country_id= destination_country_id,departure_time= departure_time)
        print(len(object))
        # it's found a flight that match to this ditails{
        #   "origin_country_id": 1,
        #   "destination_country_id": 2,
        #   "departure_time": "2022-08-22T13:53:38Z"
        # }
        return Response(FlightsSerializer().get_Flights_by_parameters(object))
...
def get_flights_by_airline_id(request):
    if request.method == 'POST': #method post add new row
        ...
        object= Flight.objects.filter(airline_company_id= airline_company_id)
        print(len(object))
        return Response(FlightsSerializer().get_Flights_by_airline_id(object))
